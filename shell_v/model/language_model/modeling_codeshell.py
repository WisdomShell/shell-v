...
class CodeShellModel(CodeShellPreTrainedModel):
    ...
    def forward(
        ...
        if attention_mask is not None:
            ...
            self_attention_mask = self_attention_mask.repeat(batch_size, 1, 1).to(dtype=torch.bool)
        # MQA models: (batch_size, query_length, n_heads, key_length)
        # MHA models: (batch_size, n_heads, query_length, key_length)
        attention_mask = self_attention_mask.unsqueeze(1)
        # print('attention_mask.shape in modeling_codeshell',attention_mask.shape)
